...
def get_molecule_alignment_mapping(bond_pairs_1, bond_pairs_2):
    from networkx.algorithms import isomorphism
    import networkx as nx
    graph_1=nx.from_edgelist(bond_pairs_1)
    graph_2=nx.from_edgelist(bond_pairs_2)
    graph_matcher=isomorphism.GraphMatcher(graph_1, graph_2)
...
def get_bond_pairs_from_xyz_file(xyz_filename):
    xyz_df=get_xyz_df_from_file(xyz_filename)
    bond_pairs=get_bond_pairs_from_xyz_df(xyz_df)
    return bond_pairs

def get_alignment_mapping_from_xyz_df(xyz_df, bond_pairs_ref):
    bond_pairs=get_bond_pairs_from_xyz_df(xyz_df)
    new_indices_mapping=get_molecule_alignment_mapping(bond_pairs_ref, bond_pairs)
    return new_indices_mapping

def align_xyz_df_mapping(xyz_df, mapping):
    actual_mapping=extract_ordered_values(mapping)            
    aligned_df=xyz_df.reindex(actual_mapping).copy().reset_index(drop=True)
    return aligned_df 
...
class OverlayAnalyzer():
    def __init__(self, parser, xyz_filenames, fit_mode='all', atoms=['1', '2']):
        ...
        """
        set_plot_settings()
##        self.xyz_filenames=xyz_filenames
        if fit_mode=='all':
...
##        self.overlay_xyzs(fit_mode)
    ...
    def get_inital_data(self):
        try:
            ...
            sys_exit(f'File(s) not found. Exit.') #file not found 

    def renumber_xyz(self):
        working_df_list=self.xyz_df_list[1:].copy()
        self.xyz_df_list=[self.xyz_df_list[0]]
        self.first_bond_pairs=get_bond_pairs_from_xyz_df(self.xyz_df_list[0])
        mapping_list=[get_alignment_mapping_from_xyz_df(xyz_df, self.first_bond_pairs) for xyz_df in working_df_list]
        aligned_df_list=[align_xyz_df_mapping(xyz_df, mapping) for xyz_df, mapping in zip(working_df_list, mapping_list)]
        self.xyz_df_list.extend(aligned_df_list)
        #index starts at 1, first atom is atom 1
        for xyz_df in self.xyz_df_list:
            xyz_df.index+=1

    def process_first_xyz(self, fit_mode):
        ...
        if fit_mode=='all':
            self.atoms_mol1=self.xyz_df_list[0][['x', 'y', 'z']]            
        if fit_mode=='atoms':
            self.atoms_mol1=self.xyz_df_list[0][['x','y','z']].iloc[self.atom_indices]
##            self.centroid0=np.mean(self.xyz_df_list[0][['x','y','z']].iloc[self.atom_indices], axis=0)        
    ...
    def overlay_xyz_by_atoms(self):
        try:
            ...
            sys_exit('Warning! Number of atoms must be equal in all xyz files. Exit.') #exit if xyz files have differnet number of atoms                        

    def overlay_xyz_by_all(self):
        try:
            for xyz_df in self.xyz_df_list:
                xyz_df[['x', 'y', 'z']], *_=rmsd_align_xyz(xyz_df[['x', 'y', 'z']], self.atoms_mol1) 
        except ValueError:
            sys_exit('Warning! Number of atoms must be equal in all xyz files. Exit.') #exit if xyz files have differnet number of atoms
...
#                
    ...
    def save_altered_xyz(self):
        try:
            for file_index, filename in enumerate(self.args.filename):
                ...
                np.savetxt(os.path.splitext(filename)[0] +'_mod.xyz',
                           my_numpy, fmt='%-2s  %12.8f  %12.8f  %12.8f', delimiter='',
                           header=self.head_list[file_index], comments='')
        except IOError: 
            sys_exit('Write error. Exit.') #write error -> exit here

    def overlay_xyzs(self, fit_mode='all', plot=False):
        self.get_inital_data()
        self.renumber_xyz()
        self.process_first_xyz(fit_mode)
        if fit_mode=='all':
    ...
    def get_lowest_rmsd_conformer_index(self):
        self.rmsd_values=np.array([rmsd_score_two_aligned_molecules_from_df(xyz_df, self.xyz_df_list[0]) for xyz_df in self.xyz_df_list[1:]])
        lowest_rmsd_conformer_index=self.rmsd_values.argmin()
        return lowest_rmsd_conformer_index

    def process_rmsd_results(self):
        data=RMSDData()
        for rmsd_index, rmsd_value in enumerate(self.rmsd_values):
            setattr(data, 'rmsd_'+str(rmsd_index), rmsd_value)
        setattr(data, 'min_rmsd', round(min(self.rmsd_values), RMSDConstants.ROUND_DIGITS.value))
        setattr(data, 'max_rmsd', round(max(self.rmsd_values), RMSDConstants.ROUND_DIGITS.value))
        setattr(data, 'avg_rmsd', round(np.mean(self.rmsd_values), RMSDConstants.ROUND_DIGITS.value))
        return data.__dict__

class TabDataAnalyzer():
    def __init__(self, parser, xyz_filename, get_plot=False, bonds_by_ref=False, bonds_ref=None):
        if get_plot:
    ...
    def get_inital_data(self):
        ...
        #expend xyz_df
        self.xyz_df=expend_xyz_df(xyz_df, index_atoms=self.args.index)
...
##        os.chdir(home_path)
    ...
    xyz_filename_1='0_1106253-mod.xyz'
    xyz_filename_2='1_1106253.xyz'
    xyz_filename_3='5_1106253.xyz'
    molecule_tab_1=TabDataAnalyzer(set_tab_parser(), xyz_filename_3, get_plot=True)
...
##    overlay_analyzer.overlay_xyzs(fit_mode='all', plot=True)
##    print(overlay_analyzer.get_lowest_rmsd_conformer_index())
