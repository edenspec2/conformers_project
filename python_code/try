...
def calc_energ(pbmol):
    energy=pbmol.calcenergy(forcefield="mmff94")
    return energy
#####
...
class Conformers():
    ...
    def __init__(self, conformers_filename, molecule_format='xyz'):
        ...
        self.extended_conformers=pd.concat([pd.DataFrame(self.conformers_list,columns=['conformer']),self.conformers_energy_list, self.dipole_list,self.sterimol_list],axis=1) #,self.conformers_energy_list
    ...
    def get_parameter_index(self,parameter, index_num=None, ascending=True):
        ...
        if index_num==None:
            ordered_df=self.extended_conformers.sort_values(parameter,axis=0,ascending=ascending)
        else:
...
class ConformersFieldAnalayzer():
   ...
   def __init__(self, conformers_object):
       ...
       self.num_conformers=len(self.conformers_class.conformers_list)
